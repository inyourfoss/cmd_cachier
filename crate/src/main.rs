...
mod database {
    pub fn connection() -> redis::Connection {
        ...
        let client: redis::Client = redis::Client::open(
...
mod server {
    ...
    pub fn start() -> bool {
        ...
        stdin.write_all(config_string.as_bytes())
        ...
        while ! (is_running()){
            ...
            std::thread::sleep(
                crate::constants::REDIS_READ_WRITE_LATENCY_IN_MS
            );
        }
        eprintln!();
...
mod cache_commands {
    ...
    pub fn save_or_query_cmd(args: Vec<String>) 
        -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn save_cmd(args: Vec<String>) 
        -> Result<(), Box<dyn std::error::Error>> {
        ...
        let cmd_stdout: String = String::from_utf8_lossy(
    ...
    pub fn redis_meminfo() 
        -> Result<(), Box<dyn std::error::Error>> {

        let mut _con = crate::database::connection();

        let used_mem_size :f64 = 
            redis::cmd("MEMORY")
            .arg("USAGE")
            .arg("cmd")
            .query(&mut _con)?;

        let to_megabytes = |i: f64| -> String { 
            format!("{:.3} KiB", i / (1024.0)) 
        };

        let to_kibibytes = |i: f64| -> String { 
            format!("{:.3} MiB", i / (1024.0 * 1024.0)) 
        };
